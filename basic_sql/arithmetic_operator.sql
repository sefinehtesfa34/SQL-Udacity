...
-- find the standard paper percent for the order, 
-- and use the AS keyword to name this new column "std_percent." 
-- You can run this query on the next page if youd like, to see the output.
...
-- from poster paper for each order. You will need to use only the columns that end with _usd. 
-- (Try to do this without using the total column.) 
-- Display the id and account_id fields also. NOTE - you will receive an error 
-- with the correct solution to this question. This occurs because 
-- at least one of the values in the data creates a division by zero in your formula. 
-- There are ways to better handle this. For now, you can just limit your calculations 
-- to the first 10 orders, as we did in question #1, and 
...
-- If you want to combine values of a particular column, across multiple rows, 
-- we will do this with aggregations. We will get to that before the end of the course!
...
-- to make this a percentage, but for the purposes of this quiz, you do not need to do this.
